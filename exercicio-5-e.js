...
const todosTemReview = inventory.every(
  (p) => Array.isArray(p.reviews) && p.reviews.length > 0
);

console.log(`Todos os produtos têm pelo menos uma avaliação? ${todosTemReview ? 'Sim' : 'Não'}`);
