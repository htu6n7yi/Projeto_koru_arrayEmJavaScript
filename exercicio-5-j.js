...
// Primeiro, calcular a média (repetido aqui porque não há conexão entre arquivos)
inventory.forEach(produto => {
    const total = produto.reviews.reduce((soma, nota) => soma + nota, 0);
    const media = total / produto.reviews.length;
...
const produtoMelhorAvaliado = inventory.reduce((melhor, atual) =>
    atual.averageRating > melhor.averageRating ? atual : melhor
);

console.log("j) Produto com maior média de avaliações:");
console.log(produtoMelhorAvaliado);
